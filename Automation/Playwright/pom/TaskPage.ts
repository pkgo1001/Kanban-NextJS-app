...
export class TaskPage {
    ...
    constructor(page: Page) {
        ...
        this.assigneeCombo = page.getByRole('combobox', { name: 'Assignee' })
    ...
    async setAssignee(assignee: string) {
        ...
        // Esperar a que el listbox esté visible (el dropdown está abierto)
        const listbox = this.page.getByRole('listbox');
        await expect(listbox).toBeVisible({ timeout: 5000 });
        
        // Esperar a que las opciones reales carguen (no "Loading...")
        // Usamos waitFor para que reintente si falla
        await this.page.waitForFunction(async () => {
            const options = document.querySelectorAll('[role="option"]');
            if (options.length === 0) return false;
            // Verificar que no hay opciones de "Loading"
            for (const opt of options) {
                if (opt.textContent?.includes('Loading')) return false;
            }
            return true;
        }, { timeout: 10000 });
        
        // Esperar a que la opción deseada esté disponible y hacer click
        const option = this.page.getByRole('option', { name: assignee, exact: true });
        ...
        // Verificar que el valor se actualizó en el combobox
        await expect(this.assigneeCombo).toContainText(assignee, { timeout: 5000 });
    }
    ...
    async clickCancel() {
        return await this.cancelTaskButton.click();
    }
